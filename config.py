...
1.3.0 起：API 开关按主名生成；旧配置中可能出现的别名开关（如
...
def _normalize_config_key(key: str) -> str:
    ...
    1.3.0 起所有 API 开关按主名生成。若旧配置文件使用别名键
...
class OnebotExpandConfig(BaseConfig):
    ...
    class ApiSwitchesSection(SectionBase):
        ...
        每个 API 拥有独立的布尔开关，格式为 ``enable_<api_name>``。
        当某 API 开关为 False 时，插件拒绝执行该 API 调用并返回禁用提示。
        """
        ...
        # --- 文件类 API (7) ---
        enable_upload_file: bool = Field(
            default=True,
            description="是否启用 upload_file API（上传文件）",
        )
        ...
        enable_get_file_url: bool = Field(
            default=True,
            description="是否启用 get_file_url API（获取文件下载 URL，NapCat 扩展）",
        )
        ...
        enable_get_group_add_request: bool = Field(
            default=True,
            description="是否启用 get_group_add_request API（获取群添加请求，NapCat扩展）",
        )
        ...
        # --- 机型/其他类 API (10) ---
        enable_get_model_show: bool = Field(
            default=True,
            description="是否启用 get_model_show API（获取机型展示）",
        )
        enable_set_model_show: bool = Field(
            default=True,
            description="是否启用 set_model_show API（设置机型展示）",
        )
